  ...
  async execute(interaction: ICommandInteraction, args: ICommandArgs) {
    switch (interaction.subcommand) {
      case "user":
        ...
        const userInfo = await this.bot.util.resolveMember(
          interaction.guild!,
          argUser
        );
        if (!userInfo) {
      ...
      case "bot":
        ...
        const embedBot = new MessageEmbed()
          .setColor(this.colors.embed)
          .setAuthor(
            `${this.bot.client.user!.username} Info`,
          ...
          .addFields(
            ...
            },
            {
              name: "Uptime",
              value: `${Math.floor(
                this.bot.client.uptime! / 1000 / 60
              ).toString()} minutes`,
              inline: true,
            },
            {
          ...
          .setTimestamp()
          .setFooter(
            `Infos of ${this.bot.client.user!.username}. BOT ID : ${
      ...
      case "role":
        ...
        break;
      case "channel":
        const channel = this.bot.util.resolveChannel(
          interaction.guild!,
          args.get("channel").value
        );
        if (!channel)
        ...
        const embedChannel = new MessageEmbed()
          ...
          .addFields(
            ...
            {
              name: "Topic :",
              value: `${(channel as TextChannel).topic || "No topic"}`,
              inline: false,
            },
            {
              name: "Category ID :",
