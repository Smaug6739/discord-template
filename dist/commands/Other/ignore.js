    ...
    execute(interaction, args, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            switch (interaction.subcommand) {
                case 'add':
                    const channelToAdd = yield this.bot.util.resolveChannel(interaction.guild, args.get('channel').value);
                    if (!channelToAdd)
                        return interaction.replyErrorMessage(`Channel not found.`);
                    else {
                        settings.ignoreChannel.push(channelToAdd.id);
                        yield this.bot.db.updateGuild(interaction.guild.id, { $push: { ignoreChannel: channelToAdd.id } });
                        interaction.replySuccessMessage(`This channel is now ignored.`);
                    }
                    break;
                case 'rem':
                    const channelToRem = this.bot.util.resolveChannel(interaction.guild, args.get('channel').value);
                    if (!channelToRem)
                        return interaction.replyErrorMessage(`Channel not found.`);
                    if (!settings.ignoreChannel.includes(channelToRem.id))
                        return interaction.replyErrorMessage(`This channel is not ignored.`);
                    yield this.bot.db.updateGuild(interaction.guild.id, { $pull: { ignoreChannel: channelToRem.id } });
