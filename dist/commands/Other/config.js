        ...
        return __awaiter(this, void 0, void 0, function* () {
            switch (interaction.subcommand) {
                case 'log-channel':
                    ...
                    if (logChannel) {
                        yield this.bot.db.updateGuild(interaction.guild, { logChannel: logChannel });
                ...
                // case 'welcomeMessage':
                // 	if (newSetting) {
                // 		await this.bot.db.updateGuild(interaction.guild, { welcomeMessage: newSetting });
                ...
                case 'experience':
                    ...
                    if (settings.expsysteme == true)
                        ...
                        uexp = true;
                    yield this.bot.db.updateGuild(interaction.guild.id, { expsysteme: uexp });
                ...
                case 'admin-invites':
                    ...
                    if (settings.invitations == true)
                        ...
                        invit = true;
                    yield this.bot.db.updateGuild(interaction.guild.id, { invitations: invit });
                ...
                case 'rank-card':
                    ...
                    if (rankCard) {
                        if (rankCard.includes('png') || rankCard.includes('PNG') || rankCard.includes('JPG') || rankCard.includes('jpg') || rankCard.includes('JPEG') || rankCard.includes('jpeg') || rankCard.includes('GIF') || rankCard.includes('gif')) {
                            yield this.bot.db.updateGuild(interaction.guild.id, { rankcard: rankCard });
                ...
                case 'rank-channel':
                    ...
                    if (rankChannel) {
                        if (rankChannel === 'disable') {
                            this.bot.db.updateGuild(interaction.guild.id, { salonranks: "" });
                            return interaction.replySuccessMessage(`Rank channel has been disable.`);
                        }
                        else {
                            const channel = this.bot.util.resolveChannel(interaction.guild, rankChannel);
                            if (!channel || channel == undefined)
                                return interaction.replyErrorMessage(`Channel not found.`);
                            else {
                                yield this.bot.db.updateGuild(interaction.guild.id, { salonranks: channel.id });
